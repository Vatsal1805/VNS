  ...
  return (
    <section className="py-20 bg-vnsLight text-vnsDark">
      <div className="max-width-container mx-auto px-5 lg:px-12">
        ...
        >
          <span className="font-heading text-xs font-semibold uppercase tracking-widest text-vnsDark/60">Core Beliefs</span>
        ...
        >
          {coreValues.map((value, idx) => (
            ...
            >
              ...
              <h4 className="font-heading font-semibold text-vnsDark text-xs md:text-sm mt-1">{value.title}</h4>
              <p className="font-body text-[10px] md:text-xs text-vnsDark/70 leading-relaxed">{value.desc}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
